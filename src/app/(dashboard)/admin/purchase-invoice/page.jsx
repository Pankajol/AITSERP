...
// Initial state now includes invoiceType.
// Options: "Normal", "POCopy", "GRNCopy"
...
export default function PurchaseInvoiceForm() {
  ...
  // Load invoice for editing if an id is provided.
  useEffect(() => {
    if (params?.id) {
      axios
  ...
  // When an item is selected, fetch its managedBy if needed.
  ...
  // Effect: Check and fetch managedBy for items that need it.
  ...
  // Updated: Convert batchQuantity to a number if needed.
  const handleBatchEntryChange = useCallback((itemIndex, batchIndex, field, value) => {
    // If the field is batchQuantity, convert value to number.
  ...
  useEffect(() => {
    ...
    const grandTotalCalc = Math.round((totalItemsCalc + gstTotalCalc)*100)/100;
    setSummary({
      ...
      if (typeof window !== "undefined") {
        ...
        if (poData) {
          try {
            ...
            // Set invoiceType to "POCopy" for PO copy.
  ...
  useEffect(() => {
    async function loadGRNCopyData() {
      if (typeof window !== "undefined") {
        const grnCopyData = sessionStorage.getItem("grnData");
        if (grnCopyData) {
          try {
            const parsedData = JSON.parse(grnCopyData);
            // Set invoiceType to "GRNCopy" for GRN copy.
  ...
  const handleSaveGRN = useCallback(async () => {
    ...
    for (let item of grnData.items) {
      ...
      if (allowedQty > 0 && Number(item.quantity) > allowedQty) {
        toast.error(`For item ${item.itemCode}, invoice quantity (${item.quantity}) exceeds allowed quantity (${allowedQty}).`);
        return;
      }
    ...
    console.log("Purchase Invoice Data:", grnData);
    try {
      const response = await axios.post("/api/purchaseInvoice", grnData, {
        headers: { "Content-Type": "application/json" },
      });
      toast.success(`Purchase Invoice saved successfully.  ID: ${response.data.invoiceId}`);
      setGrnData(initialGRNState);
    } catch (error) {
      console.error("Error saving Purchase Invoice:", error);
      toast.error(error.response?.data?.message || "Error saving Purchase Invoice");
    }
  }, [grnData]);
    ...
    <div ref={parentRef} className="m-11 p-5 shadow-xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-8 m-8 border rounded-lg shadow-lg">
        ...
        <div>
          ...
          <input type="number" name="gstTotal" value={(summary.gstTotal).toFixed(0)} readOnly className="w-full p-2 border rounded bg-gray-100" />
        </div>
        <div>
...
// // Assume initialGRNState is defined somewhere and now includes invoiceType.
// const initialGRNState = {
//   supplierName: "",
//   supplierCode: "",
//   contactPerson: "",
...
//   gstTotal: 0,
//   grandTotal: 0,
//   // New field for invoice type. Options: "Normal", "POCopy", "GRNCopy"
//   invoiceType: "Normal",
//   items: [],
//   qualityCheckDetails: [],
// };
...
//   // New effect: Fetch managedBy for items that need it.
//   const unfetchedCount = grnData.items.filter(
//     (item) => item.item && (!item.managedBy || item.managedBy.trim() === "")
//   ).length;
//   useEffect(() => {
...
//     console.log("Purchase Invoice Data:", grnData);
//     try {
//       // Post the form data. The invoiceType field is now included.
//       const response = await axios.post("/api/purchaseInvoice", grnData, {
//         headers: { "Content-Type": "application/json" },
//       });
//       toast.success(`Purchase Invoice saved successfully.  ID: ${response.data.grnId}`);
//       setGrnData(initialGRNState);
//     } catch (error) {
//       console.error("Error saving Purchase Invoice:", error);
//       toast.error(error.response?.data?.message || "Error saving Purchase Invoice");
...
//       if (typeof window !== "undefined") {
//         const poData = sessionStorage.getItem("grnData");
//         if (poData) {
//           try {
//             const parsedData = JSON.parse(poData);
//             for (let i = 0; i < parsedData.items.length; i++) {
...
//   // useEffect(() => {
//   //   console.log("purchase Invoice Items:", grnData.items);
//   // }, [grnData.items]);
...
//       }
//     }
//     console.log("purchase Invoice Items",grnData)
//     try {
//       const response = await axios.post("/api/purchaseInvoice", grnData, {
//         headers: { "Content-Type": "application/json" },
//       });
...
//     } catch (error) {
//       console.error("Error saving Purchase Invoice:", error);
//       toast.error(error.response?.data?.message || "Error saving Purchse Invoice");
//     }
//   }, [grnData]);
